...
import { AfterCreateCrudUseCaseImpl, FilterBuilder } from '@/data'

describe('After Create Crud Use Case', function() {
  const useCase = new AfterCreateCrudUseCaseImpl<any>({
  ...
  it('Should call read use case with id', async function() {
    ...
    // Act
    const receivedObject = await useCase.fetchAfterCreation(mockId)
